...
// Discord expires an unacknowledged interaction after about 3 seconds. Deferred replies keep long security actions alive.
const safeReply = async (i, data) => { try { if (i.deferred) await i.editReply(data); else if (i.replied) await i.followUp(data); else await i.reply(data); } catch (e) { console.error('Interaction reply:', e.message); } };
...
client.on(Events.InteractionCreate, async i => { if (!i.guild) return safeReply(i, { content: 'Cette commande doit être utilisée dans un serveur.', ephemeral: true }); const c = cfg(i.guild);
  ...
  if (i.isButton()) { if (!canManage(i)) return safeReply(i, { content: 'Permission Manage Server requise.', ephemeral: true }); if (i.customId === 'hp:lock') { await i.deferUpdate(); await setLockdown(i.guild, !c.lockdown, `Demandé par ${i.user.tag}`); return i.editReply(dashboardPanel(i.guild)); } if (i.customId === 'hp:status') return safeReply(i, { embeds: [statusEmbed(i.guild)], ephemeral: true }); if (i.customId === 'hp:incidents') return safeReply(i, incidentView(i.guild, true)); if (i.customId === 'hp:channels') { const modal = new ModalBuilder().setCustomId('hp:channels-modal').setTitle('Salons HelProtect'); const logs = new TextInputBuilder().setCustomId('logs').setLabel('ID salon de logs (vide = laisser)').setStyle(TextInputStyle.Short).setRequired(false).setValue(c.logChannelId || ''); const alerts = new TextInputBuilder().setCustomId('alerts').setLabel('ID salon d’alertes (vide = laisser)').setStyle(TextInputStyle.Short).setRequired(false).setValue(c.alertChannelId || ''); return i.showModal(modal.addComponents(new ActionRowBuilder().addComponents(logs), new ActionRowBuilder().addComponents(alerts))); } }
  ...
  if (i.isStringSelectMenu() && i.customId === 'hp:module') { if (!canManage(i)) return safeReply(i, { content: 'Permission Manage Server requise.', ephemeral: true }); const k = i.values[0]; c.modules[k] = !c.modules[k]; await i.update(dashboardPanel(i.guild)); void sendLog(i.guild, '⚙️ Configuration', `${k} : ${c.modules[k] ? 'activé' : 'désactivé'} par ${i.user.tag}`); return; }
  ...
  try { await i.deferReply({ ephemeral: true }); } catch (e) { console.warn('Unable to defer interaction:', e.message); return; }
