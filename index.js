...
function configPanel(guild) {
  const c = cfg(guild);
  const embed = new EmbedBuilder().setColor(base.colors.info).setTitle('⚙️ Configuration HelProtect').setDescription('Choisis une action : les sélecteurs Discord affichent directement les salons, membres et rôles du serveur.').addFields({ name: 'Salons actuels', value: `Logs : ${c.logChannelId ? `<#${c.logChannelId}>` : 'non défini'}\nAlertes : ${c.alertChannelId ? `<#${c.alertChannelId}>` : 'non défini'}` }, { name: 'Données', value: 'Les réglages sont en mémoire et seront perdus au redémarrage Railway.' });
  const select = new StringSelectMenuBuilder().setCustomId('hp:cfg-action').setPlaceholder('Que veux-tu configurer ?').addOptions(
    { label: 'Salon de logs', value: 'logs', description: 'Choisir un salon dans la liste Discord' }, { label: 'Salon d’alertes', value: 'alerts', description: 'Choisir un salon dans la liste Discord' },
    { label: 'Liste blanche — membre', value: 'whitelist-user', description: 'Ajouter/enlever un membre' }, { label: 'Liste noire — membre', value: 'blacklist-user', description: 'Ajouter/enlever un membre' },
    { label: 'Rôles staff protégés', value: 'protected-role', description: 'Choisir un rôle à protéger' }, { label: 'Modules de protection', value: 'modules', description: 'Anti-raid, anti-nuke, etc.' }, { label: 'Toutes les commandes', value: 'commands', description: 'Voir les commandes disponibles' });
  return { ephemeral: true, embeds: [embed], components: [new ActionRowBuilder().addComponents(select), new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('hp:cfg-dashboard').setLabel('Dashboard protection').setStyle(ButtonStyle.Primary))] };
}
