...
function dashboardPanel(guild) {
  ...
  const control = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('hp:lock').setLabel(c.lockdown ? 'Déverrouiller' : 'Lockdown').setStyle(c.lockdown ? ButtonStyle.Success : ButtonStyle.Danger), new ButtonBuilder().setCustomId('hp:status').setLabel('Statut').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('hp:incidents').setLabel('Incidents').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('hp:channels').setLabel('Salons').setStyle(ButtonStyle.Secondary));
...
const commands = [
  ...
  new SlashCommandBuilder().setName('config').setDescription('Configure HelProtect').addSubcommand(s => s.setName('logs').setDescription('Définit le salon de logs').addChannelOption(o => o.setName('channel').setDescription('Salon').addChannelTypes(ChannelType.GuildText).setRequired(true))).addSubcommand(s => s.setName('alerts').setDescription('Définit le salon d’alertes').addChannelOption(o => o.setName('channel').setDescription('Salon').addChannelTypes(ChannelType.GuildText).setRequired(true))).addSubcommand(s => s.setName('threshold').setDescription('Seuils anti-raid/anti-spam').addStringOption(o => o.setName('module').setDescription('Module').setRequired(true).addChoices({ name: 'Anti-Raid', value: 'antiraid' }, { name: 'Anti-Spam', value: 'antispam' })).addIntegerOption(o => o.setName('count').setDescription('Nombre').setRequired(true).setMinValue(2).setMaxValue(100))).addSubcommand(s => s.setName('show').setDescription('Affiche la configuration')),
...
client.on(Events.GuildMemberAdd, async member => { const c = cfg(member.guild); if (!c.enabled) return; const count = trimWindow(memberEvents, member.guild.id, c.antiraid.windowMs); const young = now() - member.user.createdTimestamp < c.antiraid.minimumAccountAgeMs; if (c.modules.antibot && member.user.bot) { const a = await auditActor(member.guild, AuditLogEvent.BotAdd, member.id); const dangerous = member.permissions?.any(c.antibot.dangerousPermissions) || false; if (a && !isTrusted(a.executor, c)) { const outcome = await enforce(member, dangerous ? c.antibot.action : 'LOG', 'Bot non approuvé ajouté', c); await risk(member.guild, a.executorId, dangerous ? c.risk.weights.bot : 10, 'Anti-Bot', `Bot ${member.user.tag}; ${outcome}`); } }
...
client.on(Events.InteractionCreate, async i => { if (!i.guild) return safeReply(i, { content: 'Cette commande doit être utilisée dans un serveur.', ephemeral: true }); const c = cfg(i.guild);
  ...
  if (name === 'config') { const sub = i.options.getSubcommand(); if (sub === 'logs' || sub === 'alerts') c[`${sub === 'logs' ? 'log' : 'alert'}ChannelId`] = i.options.getChannel('channel').id; else if (sub === 'threshold') { const k = i.options.getString('module'); c[k][k === 'antiraid' ? 'joins' : 'messages'] = i.options.getInteger('count'); } else return reply({ content: '```json\n' + JSON.stringify(c, null, 2).slice(0, 1800) + '\n```\nDonnées temporaires : perdues à tout redémarrage.', ephemeral: true }); return reply({ content: 'Configuration enregistrée en mémoire.', ephemeral: true }); }
